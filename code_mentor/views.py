...
class APIVersionView(APIView):
    ...
    def get(self, request):
        ...
        version = request.query_params.get('version', settings.REST_FRAMEWORK['DEFAULT_VERSION'])
        ...
        if version not in settings.REST_FRAMEWORK['VALID_VERSIONS']:
            return Response({"error": "Invalid version."}, status=status.HTTP_404_NOT_FOUND)
